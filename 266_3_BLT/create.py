...
if (int(case) == 1):
    ...
    for i in range(0,30):
        stationid = str(i+1)
        MotherString = MotherString + "<img src='./img/stationDot32.png' class='imgStation3' name='3"+ name[i] +"' style='left:" + str((i)*left) + ";top: 120px; position:absolute; width:22px' id=" + ids[i] + " left=" + str((i)*left) + "px top=120px stationid=" + stationid + " ng-click='getStationInfo($event)' track='6' posTime='' posBuss='' negBuss='' negTime=''>"
    print MotherString
...
if (int(case) == 2):
    ...
    for i in range(0,15):
        pos = i*2
        MotherString = MotherString + "<li class='imgStation3' id='3" + name[pos] + "'>" + name[pos] + "</li>"
