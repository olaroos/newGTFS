...
theFile.close()
theFile = open('./266_5_BLT_stationNames_nogap', 'r')
...
if (int(case) == 1):
    ...
    for i in range(0,27):
        stationid = str(i+1)
        MotherString = MotherString + "<img src='./img/stationDot52.png' class='imgStation5' name='5"+ namenogap[i] +"' style='left:" + str((i)*left) + ";top: 120px; position:absolute; width:22px' id='" + ids[i] + "' left=" + str((i)*left) + "px top=120px stationid=" + stationid + " ng-click='getStationInfo($event)' track='6' posTime='' posBuss='' negBuss='' negTime=''>"
    print MotherString
...
if (int(case) == 2):
    ...
    for i in range(0,14):
        pos = i*2
        MotherString = MotherString + "<li class='imgStation5' id='5" + namenogap[pos] + "'>" + name[pos] + "</li>"
