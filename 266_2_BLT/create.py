...
if (int(case) == 1):
    ...
    for i in range(0,34):
        stationid = str(i+1)
        MotherString = MotherString + "<img src='./img/stationDot22.png' class='imgStation2' name='2"+ name[i] +"' style='left:" + str((i)*left) + ";top: 120px; position:absolute; width:22px' id=" + ids[i] + " left=" + str((i)*left) + "px top=120px stationid=" + stationid + " ng-click='getStationInfo($event)' track='6' posTime='' posBuss='' negBuss='' negTime=''>"
    print MotherString
...
if (int(case) == 2):
    ...
    for i in range(0,17):
        pos = i*2
        MotherString = MotherString + "<li class='imgStation2' id='2" + name[pos] + "'>" + name[pos] + "</li>"
